...
export interface SendCongratsEmailFormProps {
  initialValues?: SendCongratsEmailFormFields;
  emailsTo: {
    value: number;
    label: string;
  }[];
  edit?: boolean;
  onSubmit: (values: SendCongratsEmailFormFields) => Promise<any>;
  onClose: () => void;
}

export interface SendCongratsEmailFormFields {
  subject: string;
  emailContent: string;
  emailTo: number[];
  week: number;
...
const defaultValues: SendCongratsEmailFormFields = {
  subject: '',
  emailContent: '',
  emailTo: [],
  week: 0,
...
const SendCongratsEmailFormFieldsSchema = Yup.object().shape({
...
export const SendCongratsEmailForm: FC<SendCongratsEmailFormProps> = ({
  initialValues,
  onSubmit,
  emailsTo = [],
  onClose,
}) => {
  const hf = useForm<SendCongratsEmailFormFields>({
    defaultValues,
    values: initialValues,
    mode: 'onBlur',
    resolver: yupResolver(SendCongratsEmailFormFieldsSchema),
  });
  ...
  return (
    <Box>
      <HitForm hf={hf} onSubmit={onSubmit}>
        ...
        >
          <ModalTitleHeader title="Send congratulation email" variant="h5" onClose={onClose} />
          <Controller
            name="emailTo"
            control={hf.control}
            rules={{ required: true }}
