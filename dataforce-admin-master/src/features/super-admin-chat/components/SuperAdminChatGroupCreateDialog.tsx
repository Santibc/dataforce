...
import {
  ISuperAdminOwner,
  useSAAllOwnersQuery,
  useSACreateChatGroupMutation,
} from 'src/api/superAdminChatRepository';
import type { ICreateChatGroup } from 'src/api/chatRepository';
...
interface FormValues {
  ...
  member_ids: ISuperAdminOwner[];
}

export function SuperAdminChatGroupCreateDialog({ open, onClose }: Props) {
  const createMutation = useSACreateChatGroupMutation();
  const { data: owners = [] } = useSAAllOwnersQuery();
            ...
            <Controller
              ...
              render={({ field }) => (
                <TextField {...field} label="Group type" select fullWidth>
                  ...
                  <MenuItem value="global">Global (all owners)</MenuItem>
                </TextField>
              )}
            />
              ...
              <Controller
                ...
                render={({ field }) => (
                  <Autocomplete
                    ...
                    getOptionLabel={(opt) =>
                      `${opt.firstname} ${opt.lastname} (${opt.company_name})`
                    }
